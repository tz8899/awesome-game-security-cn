...
- [什么是 awesome-game-security？](#-什么是-awesome-game-security)
- [如何快速上手](#-如何快速上手)
- [资源分类导航](#-资源分类导航)
- [常见使用场景](#-常见使用场景)
- [如何贡献](#-如何贡献)
...
## 🤔 什么是 awesome-game-security？

**awesome-game-security** 是一个精心整理的 **游戏安全资源合集**（awesome list），涵盖了游戏安全领域的各个方面：

| 领域 | 内容 |
|------|------|
| 🔫 **游戏作弊/反作弊** | 各种 Hook 框架、DMA 技术、反作弊系统（EAC/BE/Vanguard）分析 |
| 🔧 **逆向工程** | IDA/Ghidra/x64dbg/Cheat Engine 插件和脚本 |
| 🧠 **内核安全** | Windows 内核驱动、PatchGuard、DSE 绕过、VT-x 虚拟化 |
| 📱 **移动安全** | Android（Frida/Xposed/Magisk）/ iOS（越狱/内存工具） |
| 🎮 **游戏引擎** | Unreal/Unity/Godot 源码分析、引擎漏洞 |
| 🖥️ **AI 与 MCP** | 将 Claude/GPT 接入 IDA/Ghidra/x64dbg 的 AI 插件 |
| 🛠️ **开发工具** | 渲染（DX/OpenGL/Vulkan）、网络、数学库 |
...
直接往下滚动浏览本 README，所有资源按分类排列，点击链接即可访问。

### 方式二：安装 AI 技能（Cursor / Claude Code）

```bash
# 安装所有 AI 安全助手技能
npx skills add gh:gmh5225/awesome-game-security
```

安装后可在 Cursor/Claude Code 中使用以下 AI 命令：

| 命令 | 功能 |
|------|------|
| `@game-security` /g `你的问题` | 提问游戏安全相关问题 |
| `/code-review-asm` | 审查汇编代码 |
| `/code-review-cpp` | 审查 C++ 代码 |
| `/code-review-csharp` | 审查 C# 代码 |
| `/code-review-python` | 审查 Python 代码 |
| `/code-review-java` | 审查 Java 代码 |
| `/knowledge` | 游戏安全知识问答 |
| `/r3` | Ring3（用户态）知识问答 |
| `/r0` | Ring0（内核态）知识问答 |
...
在本页面使用 `Ctrl+F` / `Cmd+F` 搜索以下关键词快速定位：
...
### 引擎与开发
- 游戏引擎（Unreal/Unity/Godot）
- 游戏网络（客户端/服务器）
- 渲染引擎（DirectX/OpenGL/Vulkan）
- 数学库与 3D 图形
- AI / 图像编解码
...
- Cheat Engine 相关
- Frida 脚本
- 脱壳与代码混淆（VMProtect/Themida/OLLVM）
- DMA 攻击技术

### 平台安全
- Android 安全（Root/Hook/逆向）
- iOS 安全（越狱/内存操作）
- 游戏主机（Switch/PS/Xbox）
...
### 场景 1：学习游戏逆向
1. 从 **Reverse Engineering** 分类找到 IDA/Ghidra 教程
2. 学习 Cheat Engine 内存搜索基础
3. 阅读 Frida 脚本示例进行动态分析

### 场景 2：研究反作弊系统
1. 查看 **Anti-Cheat** 分类下的 EAC/BE/Vanguard 逆向资料
2. 了解内核级别的 PatchGuard 和 DSE 绕过技术
3. 学习 IOMMU/DMA 防御原理

### 场景 3：开发游戏安全工具
1. 在 **MCP Servers** 分类中找到 IDA/Ghidra 的 AI 插件
2. 使用 AI 技能辅助代码审查
3. 参考 **Game Network** 分类开发安全通信

### 场景 4：移动游戏安全
1. Android 分类下的 Xposed/Magisk/Frida 模块
2. iOS 分类下的越狱工具和内存调试
3. 了解 ARM64 汇编基础知识
...
欢迎通过 Pull Request 或 Issue 提交更多优质资源！
...
> 本仓库所有资源均来自互联网，仅用于 **安全研究、学术交流、游戏防御开发** 等合法用途。  
> 禁止将任何资源用于非法目的，包括但不限于：开发外挂、破坏游戏公平性、侵犯他人权益等。  
> 请遵守所在国家/地区的法律法规。使用者需自行承担一切法律责任。
